...
@connect(({ elevator, loading }) => ({
  elevator,
  loading: loading.effects['elevator/initDevice', 'elevator/initCompanyList', 'elevator/initProjectList', 'elevator/initDeviceFogGunTypes'],
  creating: loading.effects['elevator/saveOrUpdate']
}))
@Form.create()
class ElevatorForm extends PureComponent {
  ...
  componentDidMount() {
      ...
      new Promise((resolve) => {
        ...
        if (res.code !== 200) {
          Modal.confirm({
            ...
            cancelText: "取消",
            onOk() {
              router.push('/deviceMonitor/elevator')
            },
            onCancel() {
              router.push('/deviceMonitor/elevator')
            }
    ...
    // 初始加载公司信息
    dispatch({
        type: "elevator/initCompanyList",
        payload: {}
    });

    // 初始加载信息
    dispatch({
        type: "elevator/initProjectList",
        payload: {}
    });
  ...
  render() {
    const {
      ...
      elevator: { projects, supplierCompanys, agentCompanys, current },
      loading, creating
    } = this.props;
    const { uuid } = this.state;
