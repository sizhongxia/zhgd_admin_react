...
@connect(({ towercrane, loading }) => ({
  towercrane,
  loading: loading.effects['towercrane/initDevice', 'towercrane/initCompanyList', 'towercrane/initProjectList', 'towercrane/initDeviceFogGunTypes'],
  creating: loading.effects['towercrane/saveOrUpdate']
}))
@Form.create()
class TowercraneForm extends PureComponent {
  ...
  componentDidMount() {
      ...
      new Promise((resolve) => {
        ...
        if (res.code !== 200) {
          Modal.confirm({
            ...
            cancelText: "取消",
            onOk() {
              router.push('/deviceMonitor/towercrane')
            },
            onCancel() {
              router.push('/deviceMonitor/towercrane')
            }
    ...
    // 初始加载公司信息
    dispatch({
        type: "towercrane/initCompanyList",
        payload: {}
    });

    // 初始加载信息
    dispatch({
        type: "towercrane/initProjectList",
        payload: {}
    });
  ...
  render() {
    const {
      ...
      towercrane: { projects, supplierCompanys, agentCompanys, current },
      loading, creating
    } = this.props;
    const { uuid } = this.state;
