...
@connect(({ dustNoise, loading }) => ({
  dustNoise,
  loading: loading.effects['dustNoise/initDevice', 'dustNoise/initCompanyList', 'dustNoise/initProjectList', 'dustNoise/initDeviceFogGunTypes'],
  creating: loading.effects['dustNoise/saveOrUpdate']
}))
@Form.create()
class DustNoiseForm extends PureComponent {
  ...
  componentDidMount() {
      ...
      new Promise((resolve) => {
        ...
        if (res.code !== 200) {
          Modal.confirm({
            ...
            cancelText: "取消",
            onOk() {
              router.push('/deviceMonitor/dustNoise')
            },
            onCancel() {
              router.push('/deviceMonitor/dustNoise')
            }
    ...
    // 初始加载公司信息
    dispatch({
        type: "dustNoise/initCompanyList",
        payload: {}
    });

    // 初始加载信息
    dispatch({
        type: "dustNoise/initProjectList",
        payload: {}
    });
  ...
  render() {
    const {
      ...
      dustNoise: { projects, supplierCompanys, agentCompanys, current },
      loading, creating
    } = this.props;
    const { uuid } = this.state;
        ...
        <Card title={`${uuid === '' ? '新增' : '修改'}设备`} bordered={false} loading={loading}>
          <Form onSubmit={this.handleSubmit} layout="vertical" hideRequiredMark={false}>
            ...
            <Row gutter={24}>
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="PM2.5报警值">
                  {getFieldDecorator('pm25Alarm', {
                    initialValue: current.pm25Alarm
                  })(<InputNumber style={{ width: '100%' }} placeholder="请输入PM2.5报警值" />)}
                </Form.Item>
              </Col>
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="PM10报警值">
                  {getFieldDecorator('pm10Alarm', {
                    initialValue: current.pm10Alarm
                  })(<InputNumber style={{ width: '100%' }} placeholder="请输入PM10报警值" />)}
                </Form.Item>
              </Col>
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="噪音报警值">
                  {getFieldDecorator('noiseAlarm', {
                    initialValue: current.noiseAlarm
                  })(<InputNumber style={{ width: '100%' }} placeholder="请输入噪音报警值" />)}
                </Form.Item>
              </Col>
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="喷淋模式">
                  {getFieldDecorator('noisePattern', {
                    initialValue: current.noisePattern || "0"
                  })(
                    <Select style={{ width: '100%' }} placeholder="请选择喷淋模式">
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="数据上传周期(秒)">
                  {getFieldDecorator('noiseCycle', {
                    initialValue: current.noiseCycle
                  })(<InputNumber style={{ width: '100%' }} placeholder="数据上传周期参数值10-60" />)}
                </Form.Item>
              </Col>
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="联动继电器开合时间(分)">
                  {getFieldDecorator('noiseOc', {
                    initialValue: current.noiseOc
                  })(<InputNumber style={{ width: '100%' }} placeholder="联动继电器开合时间参数值1-10" />)}
                </Form.Item>
              </Col>
            </Row>
