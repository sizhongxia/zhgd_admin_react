...
import { saveFogGun, updateFogGun, getDeviceBaseInfoById, deleteById } from "@/services/device/fogGun";
...
export default {
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call }) {
        ...
        if(!!uuid) {
            const response = yield call(updateFogGun, payload);
            !!resolve && resolve(response)
        } else {
            const response = yield call(saveFogGun, payload);
            !!resolve && resolve(response)
        }
    },
    ...
    *initDeviceFogGunTypes({ payload }, { call, put }) {
      const response = yield call(getByType, {
        type: "FogGunType"
      });
      response.type = payload.type;
      yield put({
          type: "saveDeviceFogGunTypes",
          payload: response.data
      });
    },
  ...
  reducers: {
    ...
    saveDeviceFogGunTypes(state, action) {
      return {
        ...state,
        fogGunTypes: action.payload
      };
    },
  }
};
