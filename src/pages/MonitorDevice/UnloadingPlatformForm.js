...
@connect(({ unloadingPlatform, loading }) => ({
  unloadingPlatform,
  loading: loading.effects['unloadingPlatform/initDevice', 'unloadingPlatform/initCompanyList', 'unloadingPlatform/initProjectList', 'unloadingPlatform/initDeviceFogGunTypes'],
  creating: loading.effects['unloadingPlatform/saveOrUpdate']
}))
@Form.create()
class UnloadingPlatformForm extends PureComponent {
  ...
  componentDidMount() {
    ...
    if(!!deviceUuid) {
      ...
      new Promise((resolve) => {
        dispatch({
          type: "unloadingPlatform/initDevice",
          payload: {
            resolve,
            uuid: deviceUuid
          }
        ...
        if (res.code !== 200) {
          Modal.confirm({
            ...
            cancelText: "取消",
            onOk() {
              router.push('/deviceMonitor/unloadingPlatform')
            },
            onCancel() {
              router.push('/deviceMonitor/unloadingPlatform')
            }
      ...
      dispatch({
        type: "unloadingPlatform/cleanCurrent",
        payload: {}
      });
    }
    ...
    // 初始加载公司信息
    dispatch({
        type: "unloadingPlatform/initCompanyList",
        payload: {}
    });

    // 初始加载信息
    dispatch({
        type: "unloadingPlatform/initProjectList",
        payload: {}
    });
  ...
  handleSubmit = (e) => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      ...
      new Promise((resolve) => {
        dispatch({
          type: "unloadingPlatform/saveOrUpdate",
          payload: {
            resolve,
            uuid,
            ...values
  ...
  render() {
    const {
      ...
      unloadingPlatform: { projects, supplierCompanys, agentCompanys, current },
      loading, creating
    } = this.props;
    const { uuid } = this.state;
...
export default UnloadingPlatformForm;
